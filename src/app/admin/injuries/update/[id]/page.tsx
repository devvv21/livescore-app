import InjuryModel, { injuryStatuses } from "@/models/Injury";
...
export default async function UpdateInjuryPage({ params }: { params: { id: string } }) {
  ...
  if (!injury) return <div>Injury not found.</div>;
        ...
        <form action={updateInjuryAction}>
          ...
          <div className="space-y-6">
            <div>
              ...
              <select id="status" name="status" required defaultValue={injury.status} className="w-full bg-[#1A222D] text-white border border-gray-600 rounded-md p-3">
            ...
            <div>
              ...
              <input type="text" id="details" name="details" required defaultValue={injury.details} className="w-full bg-[#1A222D] text-white border border-gray-600 rounded-md p-3"/>
            </div>
            <div>
              <label htmlFor="returnDate" className="block text-sm font-medium text-gray-300 mb-1">Return Date</label>
              <input type="text" id="returnDate" name="returnDate" defaultValue={injury.returnDate} className="w-full bg-[#1A222D] text-white border border-gray-600 rounded-md p-3"/>
            </div>
            <div>
              <button type="submit" className="w-full bg-blue-600 text-white font-bold py-3 px-4 rounded-md hover:bg-blue-700">Update Record</button>
            </div>
          </div>
        </form>
      </div>
