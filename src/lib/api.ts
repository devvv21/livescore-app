...
const footballServerOptions: RequestInit = {
  method: 'GET',
  headers: {
    'x-apisports-key': FOOTBALL_API_KEY as string
  },
  next: { revalidate: 3600 } // Default cache revalidation: 1 hour
};

export interface ITransferApiResponse {
  player: {
    id: number;
    name: string;
    photo: string;
...
export async function fetchTransfersFromApi(leagueId: string): Promise<ITransferApiResponse[]> {
  if (!FOOTBALL_API_KEY) {
    console.error("API key is missing.");
    return [];
  }
  const url = `${FOOTBALL_API_URL}/transfers?league=${leagueId}&season=2024`;
  try {
    const response = await fetch(url, footballServerOptions);
    if (!response.ok) {
      console.error("Failed to fetch transfers from API:", response.statusText);
      return [];
    }
...
export const fetchAllPlayersInLeague = cache(async (leagueId: string, season: string = "2024"): Promise<any[]> => {
  ...
  try {
    const teamsResponse = await fetch(`${FOOTBALL_API_URL}/teams?league=${leagueId}&season=${season}`, footballServerOptions);
    if (!teamsResponse.ok) {
    ...
    const allSquadPromises = teams.map((team: any) =>
      fetch(`${FOOTBALL_API_URL}/players/squads?team=${team.team.id}`, footballServerOptions).then(res => res.json())
    );
...
export interface PlayerDetails {
  ...
  statistics: { goals: number; assists: number; saves: number | null; };
};
...
export const fetchPlayerDetails = cache(async (playerId: string, season: string = "2024"): Promise<PlayerDetails | null> => {
  ...
  try {
    ...
    const mappedPlayer: PlayerDetails = {
      ...
      league: { id: stats.league.id, name: stats.league.name, country: stats.league.country, logo: stats.league.logo, flag: stats.league.flag },
      games: { appearences: stats.games.appearences || 0, lineups: stats.games.lineups || 0, minutes: stats.games.minutes || 0, position: stats.games.position || 'N/A', rating: parseFloat(stats.games.rating || '0').toFixed(1) },
      statistics: { goals: stats.goals.total || 0, assists: stats.goals.assists || 0, saves: stats.goals.saves }
    };
    return mappedPlayer;
  } catch (error) {
...
export const getTeamDetails = async (teamId: number) => {
  ...
  if (teamFromDb.detailsLastUpdatedAt) {
    ...
    if (ageInHours < CACHE_DURATION_HOURS) isCacheValid = true;
  }
  if (isCacheValid) {
  ...
  try {
    const [fixtures, squad] = await Promise.all([
      fetchTeamFixtures(teamId.toString()),
      fetchTeamSquad(teamId.toString()),
    ]);
    const updatedTeam = await Team.findOneAndUpdate(
      { apiId: teamId },
      { $set: { fixtures: fixtures, squad: squad, detailsLastUpdatedAt: new Date() } },
      { new: true }
    );
    ...
    if (statsRes.ok) {
      ...
      if (statsData.response && statsData.response.length > 0) {
        ...
        allStatTypes.forEach(type => {
          processedStats[type] = {
            home: homeStatsRaw.find((s: any) => s.type === type)?.value ?? 0,
            away: awayStatsRaw.find((s: any) => s.type === type)?.value ?? 0
          };
        });
...
export const getMatchHighlights = cache(async (matchDetails: MatchDetails): Promise<Highlight[]> => {
  ...
  const params = new URLSearchParams({
    part: 'snippet', q: searchQuery, key: YOUTUBE_API_KEY, type: 'video', maxResults: '5', videoEmbeddable: 'true',
  });
...
export const fetchMatchLineups = cache(async (fixtureId: string): Promise<MatchLineupData | null> => {
  ...
  try {
    ...
    const processLineup = (lineupData: any): Lineup => ({
      team: { id: lineupData.team.id, name: lineupData.team.name, logo: lineupData.team.logo },
    ...
    return {
      home: processLineup(data.response[0]),
      away: processLineup(data.response[1]),
    };
  } catch (error) {
...
export const fetchDailyMatchesAndHighlights = async (date: string): Promise<DailyPageData> => {
  await dbConnect();
  const defaultResponse = { liveWithStreams: [], upcomingMatches: [], finishedWithHighlights: [] };
  ...
  const resultsFromDb = highlightsFromDb.map(dbHighlight => ({
    match: {
      ...
      status: 'FT' as const, time: 'FT'
    },
    highlight: { id: dbHighlight.highlightId, title: dbHighlight.highlightTitle, embedUrl: dbHighlight.highlightEmbedUrl }
  }));
  ...
  const apiPromises = matchesToFetch.map(async ({ match, fixture }) => {
    ...
    if (highlights && highlights.length > 0) {
      ...
      }).save().catch(e => console.error(`[DB Save Error] ${e.message}`));
