import { fetchInjuriesFromApi, fetchTeamOfTheWeek, fetchTopLeagues, fetchTransfersFromApi } from "@/lib/api";
...
import PublicTransfersTable from "@/components/PublicTransfersTable";
...
export const metadata: Metadata = {
  ...
  keywords: ['Football Scout', 'Player Transfers', 'Injury News'],
  authors: [{ name: 'TodayLiveScores' }],
  publisher: 'TodayLiveScores',
  robots: { index: true, follow: true },
  alternates: { canonical: 'https://todaylivescores.com/football-scout' },
};
...
export default async function FootballScoutPage() {
  await dbConnect();

  const [
    manualInjuries, apiInjuriesResponse, manualTransfers, apiTransfersResponse,
    teamOfTheWeek, allNews, topLeagues, relatedPosts
  ] = await Promise.all([
  ...
  const combinedInjuries = [
    ...(manualInjuries || []).map(injury => ({ ...injury, _id: injury._id.toString(), lastUpdated: injury.lastUpdated })),
    ...(apiInjuriesResponse || []).filter(api => api && api.player && !(manualInjuries || []).some(m => m.playerId === api.player.id))
      .map((api): IInjuryData | null => {
        if (!api.team || !api.injury || !api.fixture) return null;
        return {
          _id: `api-${api.player.id}`, playerId: api.player.id, playerName: api.player.name, playerPhoto: api.player.photo,
          teamId: api.team.id, teamName: api.team.name, teamLogo: api.team.logo,
          status: (api.injury.type === 'Suspended' || api.injury.type === 'Red Card') ? 'Disciplinary' : 'Injured',
          details: api.injury.reason, returnDate: 'Unknown', lastUpdated: new Date(api.fixture.date), isApiEntry: true,
        };
      }).filter(Boolean) as IInjuryData[]
  ].sort((a, b) => new Date(b.lastUpdated).getTime() - new Date(a.lastUpdated).getTime());
  
  const combinedTransfers = [
      ...(manualTransfers || []).map(t => ({...t, _id: t._id.toString(), transferDate: t.transferDate})),
      ...(apiTransfersResponse || []).filter(api => api && api.player && !(manualTransfers || []).some(m => m.playerId === api.player.id))
        .map((api): ITransfer | null => {
            const transferInfo = api.transfers?.[0];
            return transferInfo ? {
              _id: `api-${api.player.id}`, playerId: api.player.id, playerName: api.player.name, playerPhoto: api.player.photo,
              transferType: transferInfo.type, transferDate: new Date(transferInfo.date), fromTeamId: transferInfo.teams.out.id,
              fromTeamName: transferInfo.teams.out.name, fromTeamLogo: transferInfo.teams.out.logo, toTeamId: transferInfo.teams.in.id,
              toTeamName: transferInfo.teams.in.name, toTeamLogo: transferInfo.teams.in.logo, isApiEntry: true,
            } : null
        ...
        <div className="lg:flex lg:gap-8">
          ...
          <main className="w-full lg:flex-1 lg:order-2 lg:min-w-0">
            <div className="max-w-7xl mx-auto">
              ...
              <TabbedTables
                injuryTable={<PublicInjuriesTable injuries={combinedInjuries} />}
                transferTable={<PublicTransfersTable transfers={combinedTransfers} />}
              />
          ...
          <aside className="hidden lg:block lg:w-72 lg:order-3 flex-shrink-0 lg:sticky lg:top-8 lg:self-start">
            <RightSidebarScout initialTopLeagues={topLeagues} initialFeaturedMatch={null} />
            <div className="mt-3">
